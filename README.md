...
dev-pm 的 §7.1 委派表和 §6 编排模式中预留了项目特定技能的插槽。以下技能与具体项目绑定，**不随 dev-pm 发布**，用户应替换为自己的对应技能：

| 插槽 | 用途 | 替换说明 |
|------|------|----------|
| DB 安全操作 | Step 6 涉及数据库变更时 | 替换为你项目的 DB 操作技能（如 `nfmd-db-ops`、`my-project-db-tools` 等）。无 DB 场景时跳过 |
| 编排器 Worker | §6 多数据源/多处理路径 | 替换为你项目的专线 agent（如文献提取器、数据清洗器等）。单路径项目跳过 |

### 触发场景

当用户提出要**开发新功能或新项目**时自动触发，典型表达：

> 帮我开发一个 XX / 做一个 XX 功能 / 启动 XX 项目 / start a new project

不需要记住触发词——只要意图是"从零开始做一个东西"，dev-pm 就会接管流程。

## 评估轨迹

基于 [Darwin Skill 9 维度 Rubric](https://arxiv.org/abs/2605.23899)（SkillLens）评估，独立盲测验证，总分 **93.0/100**。

```
59.5 (基线) → 82.25 (R1) → 87.2 (R2) → 93.0 (R3)
```

详细的 9 维度评分表和实战案例见 [v0.1.0 Release Notes](https://github.com/Etoile04/dev-pm/releases/tag/v0.1.0)。
